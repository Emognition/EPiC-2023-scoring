...
from settings import DETAILED_SCORES_DIR, SCORED_FILES_MEMORY_DIR, DEFAULT_SUBMISSIONS_DIR, TEST_SETS_DEFAULT_DIR, FINAL_SUBMISSIONS_DIR
...
scored_files_memory_dir = SCORED_FILES_MEMORY_DIR / ("final" if args.final else "trial")
detailed_scores_memory_dir = DETAILED_SCORES_DIR / ("final" if args.final else "trial")

scoring_save_dir = "./"

print("FINAL PROCESSING" if args.final else "trial processing")

if __name__ == "__main__":
    script_start_time_str = datetime.now().strftime("%Y-%m-%dT%H:%M:%S")
    # select files to score from the submissions directory
    files_to_score = select_files_to_score(submissions_dir, scored_files_memory_dir)
